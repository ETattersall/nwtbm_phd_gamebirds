...
length(unique(cam_df$location)) ## 706 - fewer than ARUs (742)
...
gb_site_spp_cams <- site_species_cams %>%
...
gb_cams_naive_long <- gb_site_spp_cams %>%
...
gb_cams_naive_summary <- gb_cams_naive_long %>%
  group_by(study_area, species_common_name) %>%
...
win.graph()
f_cam_gb_naive <- ggplot(gb_cams_naive_summary, aes(x = reorder(study_area, -naive_occupancy), y = naive_occupancy, fill = species_common_name)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ species_common_name, scales = "free_y") +
  theme_minimal() +
  labs(
...
### Save plot
ggsave("figures/cam_naive_occupancy_by_spp_20260204.jpeg", plot = f_cam_gb_naive, width = 10, height = 6)
